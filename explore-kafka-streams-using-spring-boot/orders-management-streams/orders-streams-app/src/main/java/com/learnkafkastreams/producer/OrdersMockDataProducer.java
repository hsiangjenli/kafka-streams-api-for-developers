...
public class OrdersMockDataProducer {
  ...
  private static void publishOrdersForGracePeriod(ObjectMapper objectMapper, List<Order> orders) {
    ...
    // With Grace Period
    // [general_orders_revenue_window]: , TotalRevenue[locationId=store_4567, runnuingOrderCount=1,
    // runningRevenue=27.00]
    // [general_orders_revenue_window]: TotalRevenue[locationId=store_1234, runnuingOrderCount=1,
    // runningRevenue=27.00]
    // [general_orders_revenue_window]: TotalRevenue[locationId=store_4567, runnuingOrderCount=1,
    // runningRevenue=27.00]
    // [general_orders_revenue_window]: TotalRevenue[locationId=store_4567, runnuingOrderCount=1,
    ...
    var generalOrdersWithCustomTime =
        orders.stream().filter(order -> order.orderType().equals(OrderType.GENERAL))
            .map(order -> new Order(order.orderId(), order.locationId(), order.finalAmount(),
                order.orderType(), order.orderLineItems(), localDateTime))
            .toList();

    var generalOrders =
        orders.stream().filter(order -> order.orderType().equals(OrderType.GENERAL)).toList();
  ...
  private static List<Order> buildOrdersToTestGrace() {
    ...
    var order1 = new Order(12345, "store_1234", new BigDecimal("27.00"), OrderType.GENERAL,
        orderItems, LocalDateTime.parse("2023-02-27T08:45:58")
    // LocalDateTime.now(ZoneId.of("UTC"))
    );

    var order2 = new Order(54321, "store_1234", new BigDecimal("15.00"), OrderType.RESTAURANT,
        orderItemsRestaurant, LocalDateTime.parse("2023-02-27T08:45:58")
    // LocalDateTime.now(ZoneId.of("UTC"))
    );
