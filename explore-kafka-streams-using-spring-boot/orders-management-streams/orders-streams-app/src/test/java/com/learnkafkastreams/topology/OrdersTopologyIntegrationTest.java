...
public class OrdersTopologyIntegrationTest {
  ...
  @Autowired
  OrderWindowsService orderWindowsService;
  ...
  @Test
  void ordersRevenue_multiOrdersByWindows() throws InterruptedException {

    publishOrders();
    publishOrders();

    Awaitility.await().atMost(10, SECONDS).pollDelay(Duration.ofSeconds(1)).ignoreExceptions()
        .until(() -> orderWindowsService.getOrderRevenueWindowsByType(GENERAL_ORDERS).size(),
            equalTo(1));

    Awaitility.await().atMost(10, SECONDS).pollDelay(Duration.ofSeconds(1)).ignoreExceptions()
        .until(() -> orderWindowsService.getOrderRevenueWindowsByType(RESTAURANT_ORDERS).size(),
            equalTo(1));

    var generalOrdersRevenue = orderWindowsService.getOrderRevenueWindowsByType(GENERAL_ORDERS);
    var restaurantOrderRevenue =
        orderWindowsService.getOrderRevenueWindowsByType(RESTAURANT_ORDERS);
    ...
    assertEquals(new BigDecimal("54.00"),
        generalOrdersRevenue.get(0).totalRevenue().runningRevenue());
    System.out.println("🚀 Excepted : " + generalOrdersRevenue.get(0).startWindow());
    System.out.println("🚀 Excepted : " + generalOrdersRevenue.get(0).endWindow());

    assert generalOrdersRevenue.get(0).startWindow().isEqual(exceptedStartTime);
    assert generalOrdersRevenue.get(0).endWindow().isEqual(exceptedEndTime);

    assertEquals(new BigDecimal("30.00"),
        restaurantOrderRevenue.get(0).totalRevenue().runningRevenue());
    assert restaurantOrderRevenue.get(0).startWindow().isEqual(exceptedStartTime);
    assert restaurantOrderRevenue.get(0).endWindow().isEqual(exceptedEndTime);
  }
