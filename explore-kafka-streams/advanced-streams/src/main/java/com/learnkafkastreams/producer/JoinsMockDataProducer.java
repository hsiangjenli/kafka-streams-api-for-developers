...
public class JoinsMockDataProducer {
  ...
  private static void publishMessagesToSimulateGrace(
      Map<String, String> alphabetMap, String topicName, int delaySeconds)
      throws InterruptedException {
    var producerRecords = new ArrayList<ProducerRecord<String, String>>();
    alphabetMap.forEach(
        (key, value) ->
            producerRecords.add(
                new ProducerRecord<>(topicName, 0, now().toEpochMilli(), key, value)));

    Thread.sleep(delaySeconds * 1000L);
    ProducerUtil.publishMessageSync(producerRecords);
  }

  private static void publishMessagesWithDelay(
      Map<String, String> alphabetMap, String topic, int delaySeconds) {
    alphabetMap.forEach(
        (key, value) -> {
