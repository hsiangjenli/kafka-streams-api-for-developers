...
public class OrdersTopology {
        ...
        public static final String RESTAURANT_ORDERS_COUNT_WINDOWS =
                        "restaurant_order_count_windows";
        ...
        public static Topology buildTopology() {
                ...
                orderStream.split(Named.as("General-Restaurant-Stream")).branch(generalPredicate,
                                Branched.withConsumer(generalOrderStream -> {
                                        // generalOrderStream
                                        // .mapValues((readyOnlyKey, value) ->
                                        // revenueMapper.apply(value))
                                        // .to(
                                        // GENERAL_ORDERS,
                                        // Produced.with(Serdes.String(),
                                        // SerdesFactory.revenueSerde()));
                                        generalOrderStream.print(Printed.<String, Order>toSysOut()
                                                        .withLabel("generalStream"));
                                        aggregateOrdersByCount(generalOrderStream,
                                                        GENERAL_ORDERS_COUNT);
                                        aggregateOrdersCountByTimeWindows(generalOrderStream,
                                                        GENERAL_ORDERS_COUNT_WINDOWS, storesTable);
                                        aggregateOrderByRevenue(generalOrderStream,
                                                        GENERAL_ORDERS_REVENUE, storesTable);
                                        aggregateOrdersRevenueByTimeWindows(generalOrderStream,
                                                        GENERAL_ORDERS_REVENUE_WINDOWS,
                                                        storesTable);
                                })).branch(restaurnatPredicate,
                                                Branched.withConsumer(restaurantOrderStream -> {
                                                        ...
                                                        // Produced.with(Serdes.String(),
                                                        // SerdesFactory.revenueSerde()));
                                                        restaurantOrderStream.print(Printed
                                                                        .<String, Order>toSysOut()
                                                                        .withLabel("restaurnatStream"));
                                                        aggregateOrdersByCount(
                                                                        restaurantOrderStream,
        ...
        private static void aggregateOrdersRevenueByTimeWindows(
                ...
                Aggregator<String, Order, TotalRevenue> aggregator = (key, value,
                                aggregate) -> aggregate.updateRunningRevenue(key, value);
                KTable<Windowed<String>, TotalRevenue> revenueTable = generalOrderStream
                                .map((key, value) -> KeyValue.pair(value.locationId(), value))
                                .groupByKey(Grouped
                                                .with(Serdes.String(), SerdesFactory.orderSerde()))
                                .windowedBy(timeWindows)
                                .aggregate(totoalRevenueInitializer, aggregator, Materialized
                                                .<String, TotalRevenue, WindowStore<Bytes, byte[]>>as(
                                                                storeName)
        ...
        public static void aggregateOrdersByCount(KStream<String, Order> generalOrderStream,
                        String storeName) {
                KTable<String, Long> orderCountPerStore = generalOrderStream
                                .map((key, value) -> KeyValue.pair(value.locationId(), value))
                                .groupByKey(Grouped.with(Serdes.String(),
                                                SerdesFactory.orderSerde()))
                                .count(Named.as(storeName), Materialized.as(storeName));
        ...
        public static void aggregateOrderByRevenue(KStream<String, Order> generalOrderStream,
                ...
                KTable<String, TotalRevenue> revenueTable = generalOrderStream
                                ...
                                .groupByKey(Grouped.with(Serdes.String(),
                                                SerdesFactory.orderSerde()))
                                .aggregate(totalRevenueInitializer, aggregator, Materialized
                                                .<String, TotalRevenue, KeyValueStore<Bytes, byte[]>>as(
                                                                storeName)
                                                .withKeySerde(Serdes.String())
                                                .withValueSerde(SerdesFactory.totalRevenueSerde()));
                revenueTable.toStream().print(
                                Printed.<String, TotalRevenue>toSysOut().withLabel(storeName));
                ValueJoiner<TotalRevenue, Store, TotalRevenueWithAddress> valueJoiner =
                                TotalRevenueWithAddress::new;
        ...
        private static void printLocalDateTimes(Windowed<String> key, Long value) {
                ...
                LocalDateTime startLDT = LocalDateTime.ofInstant(startTime,
                                ZoneId.of(ZoneId.SHORT_IDS.get("CST")));
                LocalDateTime endLDT = LocalDateTime.ofInstant(endTime,
                                ZoneId.of(ZoneId.SHORT_IDS.get("CST")));
